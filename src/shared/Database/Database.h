...
#ifndef __LEGACY_DATABASE_H
#define __LEGACY_DATABASE_H
...
class LEGACY_DLL_SPEC Database
{
	protected:
	...
	public:
		...
		// Write SQL commands to a LOG file (see legacyd.conf "LogSQL")
		bool PExecuteLog(const char *format,...) ATTR_PRINTF(2,3);

		virtual bool BeginTransaction()                     // nothing do if DB not support transaction
		{
			return true;
		}
		virtual bool CommitTransaction()                    // nothing do if DB not support transaction
		{
			return true;
		}
