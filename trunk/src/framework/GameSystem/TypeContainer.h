...
#ifndef __LEGACY_TYPECONTAINER_H
#define __LEGACY_TYPECONTAINER_H
 ...
 * By itself its meaningless but collaborate with TypeContainers,
...
/*
 * @class ContainerArrayList is a multi-type container for
 * array of elements.
 */
...
class LEGACY_DLL_DECL TypeMapContainer
{
	public:
		template<class SPECIFIC_TYPE> size_t Count() const { return LeGACY::Count(i_elements, (SPECIFIC_TYPE*)NULL); }

		template<class SPECIFIC_TYPE> SPECIFIC_TYPE* find(OBJECT_HANDLE hdl, SPECIFIC_TYPE *fake) { return LeGACY::Find(i_elements, hdl, fake); }

		/// find a specific type of object in the container
		template<class SPECIFIC_TYPE> const SPECIFIC_TYPE* find(OBJECT_HANDLE hdl, SPECIFIC_TYPE *fake) const { return LeGACY::Find(i_elements, hdl, fake); }

		/// inserts a specific object into the container
		template<class SPECIFIC_TYPE> bool insert(OBJECT_HANDLE hdl, SPECIFIC_TYPE *obj)
		{
			SPECIFIC_TYPE* t = LeGACY::Insert(i_elements, obj, hdl);
			return (t != NULL);
		}
		...
		{
			SPECIFIC_TYPE* t = LeGACY::Remove(i_elements, obj, hdl);
			return (t != NULL);
		}
